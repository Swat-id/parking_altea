...
const Schedules = () => {
  ...
  const { data: schedulesData = { schedules: [] }, isLoading, refetch } = useQuery(
    ...
    () => {
      ...
      return fetch(`/api/schedules?${params}`).then(res => res.json())
    },
    {
  ...
  const updateScheduleMutation = useMutation(
    ({ id, data }) => fetch(`/api/schedules/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    }).then(res => res.json()),
  ...
  const deleteScheduleMutation = useMutation(
    (id) => fetch(`/api/schedules/${id}`, { method: 'DELETE' }).then(res => res.json()),
    {
      onSuccess: (data) => {
        if (data.success) {
  ...
  const toggleScheduleMutation = useMutation(
    (id) => fetch(`/api/schedules/${id}/toggle`, { method: 'POST' }).then(res => res.json()),
    {
      onSuccess: (data) => {
        if (data.success) {
  ...
  const executeScheduleMutation = useMutation(
    (id) => fetch(`/api/schedules/${id}/execute`, { method: 'POST' }).then(res => res.json()),
    {
      onSuccess: (data) => {
        if (data.success) {
