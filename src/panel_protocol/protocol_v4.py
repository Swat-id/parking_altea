#!/usr/bin/env python3
"""
Módulo para comunicación con paneles LED mediante protocolo Network TCP (v4)
Puerto: 5200
Protocolo: Rotuloselectronicos.net

Este módulo es complementario a la estructura existente en panel_protocol/
y proporciona una interfaz simplificada para el protocolo Tipo 4.
"""
...
class PanelProtocolV4:
    """
    Clase para construir y enviar paquetes de protocolo a paneles LED (v4)
    ...
    def build_text_packet(
        ...
        """
        # Validar window_number
        if window_number < 0 or window_number > 15:
            raise ValueError(f"window_number debe estar entre 0 y 15, recibido: {window_number}")
    ...
    def send_packet(self, packet: bytes, verbose: bool = False) -> Optional[bytes]:
        """
    ...
    def send_text(
        ...
        Args:
            text: Texto a enviar
            window_number: Número de ventana (0-15)
            color: Color del texto
            font_size: Tamaño de la fuente
            alignment: Alineación del texto
            effect: Efecto de visualización
    ...
    def send_hex_string(self, hex_string: str, verbose: bool = False) -> bool:
        """
