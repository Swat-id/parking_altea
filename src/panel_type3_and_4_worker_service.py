#!/usr/bin/env python3
"""
Servicio independiente para el worker de actualización de paneles Tipo 3 y Tipo 4
Ejecuta como proceso separado y actualiza paneles Tipo 3 (2 ventanas) y Tipo 4 (16 ventanas)
según intervalos configurados en UserPanelConfig
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/panel_type3_and_4_worker.log'),
...
class PanelType3And4WorkerService:
    """Servicio principal para el worker de paneles Tipo 3 y Tipo 4"""
    ...
    def __init__(self, update_interval: int = 120):
        ...
        logger.info(f"Panel Type 3/4 Worker Service inicializado - Intervalo: {update_interval}s")
    ...
    def start(self):
        """Iniciar el servicio"""
        logger.info("=== Iniciando Panel Type 3/4 Worker Service ===")
        
        try:
            # Iniciar el worker
            self.worker.start()
            logger.info("Panel Type 3/4 Worker iniciado correctamente")
            ...
            logger.error(f"Error crítico en Panel Type 3/4 Worker Service: {e}")
            return False
    ...
    def _log_startup_info(self):
        ...
        logger.info("Panel Type 3/4 Worker Service configuración:")
    ...
    def _log_statistics(self):
        ...
        try:
            ...
            logger.info("=== Estadísticas Panel Type 3/4 Worker ===")
            logger.info(f"Uptime: {stats.get('uptime_formatted', 'N/A')}")
            logger.info(f"Ciclos completados: {stats['cycles_completed']}")
            logger.info(f"Paneles procesados: {stats['panels_processed']}")
            logger.info(f"Paneles actualizados: {stats['panels_updated']}")
            logger.info(f"Paneles fallidos: {stats['panels_failed']}")
            logger.info(f"Ventanas actualizadas: {stats['windows_updated']}")
            logger.info(f"Paneles Tipo 3 actualizados: {stats['type3_panels_updated']}")
            logger.info(f"Paneles Tipo 4 actualizados: {stats['type4_panels_updated']}")
            logger.info(f"Tiempo promedio por ciclo: {stats['average_cycle_time']:.2f}s")
            logger.info(f"Errores: {stats['errors']}")
            
            if stats['panels_processed'] > 0:
                success_rate = (stats['panels_updated'] / stats['panels_processed']) * 100
    ...
    def _shutdown(self):
        ...
        logger.info("Iniciando shutdown del Panel Type 3/4 Worker Service...")
        ...
        try:
            ...
            # Detener el worker
            self.worker.stop()
            logger.info("Panel Type 3/4 Worker detenido correctamente")
            
        except Exception as e:
            logger.error(f"Error durante shutdown: {e}")
        
        logger.info("Panel Type 3/4 Worker Service detenido")
    ...
    def _log_final_statistics(self):
        ...
        try:
            ...
            logger.info(f"Total paneles procesados: {stats['panels_processed']}")
            logger.info(f"Total paneles actualizados: {stats['panels_updated']}")
            logger.info(f"Total ventanas actualizadas: {stats['windows_updated']}")
    ...
    def _save_final_stats_to_file(self, stats: dict):
        ...
        try:
            ...
            json_stats['service_version'] = 'v4.3.0'
            
            # Guardar en archivo
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f'logs/panel_type3_and_4_worker_stats_{timestamp}.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Panel Type 3/4 Worker Service v4.3.0')
    parser.add_argument(
        '--interval', 
        type=int, 
        default=120, 
    ...
    if args.stats:
        try:
            ...
            worker = PanelType3And4Worker(update_interval=interval)
            stats = worker.get_stats()
            
            print("=== Panel Type 3/4 Worker Service Stats ===")
    ...
    service = PanelType3And4WorkerService(update_interval=interval)
